...
public struct TimeOffsetPickerView: View {
    ...
    private let yearData = Array(0 ..< 100).map { String(localized: "\($0)年") }
    private let monthData = Array(0 ..< 100).map { String(localized: "\($0)月") }
    private let dayData = Array(0 ..< 100).map { String(localized: "\($0)天") }
    private let hourData = Array(0 ..< 24).map { String(localized: "\($0)小时") }
    private let minuteData = Array(0 ..< 60).map { String(localized: "\($0)分") }
    private let secondData = Array(0 ..< 60).map { String(localized: "\($0)秒") }
