...
//    var background: Background
    ...
    @Binding var selected: MovableObject? // Currently selected movable object

    let coordinateSpaceId = UUID()

    // Initializer with default parameters and binding
    init(model: WidgetPhase, widgetSize: CGSize, enableModify: Bool = true, selected: Binding<MovableObject?> = .constant(nil)) {
            ...
            .overlay {
                ZStack {
                    ...
                    ForEach(phase.stickers) { sticker in
                        MovableObjectView3(item: sticker, selection: $selected, config: stickerConfig) { item in
                    ...
                    ForEach(phase.texts) { text in
                        MovableObjectView3(item: text, selection: $selected, config: textConfig) { item in
                            item.view()
                        }
                    }
                    ...
                    ForEach(phase.eventInfo) { eventInfo in
                        ...
                        MovableObjectView3(item: eventInfo, selection: $selected, config: eventInfoConfig) { _ in
