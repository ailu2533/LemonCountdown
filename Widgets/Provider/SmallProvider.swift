...
struct SmallProvider: AppIntentTimelineProvider {
    func placeholder(in context: Context) -> SmallEntry {
        SmallEntry(date: Date(), configuration: SmallConfigurationAppIntent(), widgetTemplateId: nil)
    }

    func snapshot(for configuration: SmallConfigurationAppIntent, in context: Context) async -> SmallEntry {
        SmallEntry(date: Date(), configuration: configuration, widgetTemplateId: nil)
    }

    func timeline(for configuration: SmallConfigurationAppIntent, in context: Context) async -> Timeline<SmallEntry> {
        ...
        return Timeline(entries: [SmallEntry(date: Date(), configuration: .init(), widgetTemplateId: nil)], policy: .after(nextRefreshDate))
    }

    // 生成 Entries
    func generateEntries(for configuration: SmallConfigurationAppIntent, event: WidgetEntityQuerySmall, widgetTemplate: WidgetTemplate) -> [SmallEntry] {
        // 获取当前日期
        ...
        func addEntry(date: Date, phase: WidgetPhase?) {
            // 只有当阶段存在时才添加条目
            if let phase = phase {
                entries.append(SmallEntry(date: date, configuration: configuration, widgetTemplateId: event.widgetTemplateUUID, phase: phase))
            }
        }
        ...
        entries.forEach { entry in
            entry.phase?.setEventInfoProvider(event as?EventInfoProvider)
        }
